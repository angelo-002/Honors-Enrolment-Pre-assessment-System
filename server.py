...
from databaseconnectors import create_connection
    ...
    def test(self, person_id, first_name, last_name, email):
        ...
        if connection:
            try:
                ...
                cursor.execute(
                    "SELECT * FROM dbo.OUST_Student WHERE Person_ID = ? AND First_Name=? AND Last_Name=? AND OUST_Email=?",
                    (int(person_id), first_name.lower(), last_name.lower(), email.lower()))
                ...
                if (user_exists_in_database == True):
                    ...
                    cursor.execute("SELECT Course_Code, Marks FROM dbo.OUST_Student_Course WHERE Person_ID = ?",
                                   (person_id))
                    rows = cursor.fetchall()
                    self.data = []
                    for row in rows:
